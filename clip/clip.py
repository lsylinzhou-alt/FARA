...
def _download(url: str, root: str = "/public/home/hpc202520500193/GIFT/cache/clip"):
    ...
    # 若不存在，再尝试下载（可选：注释掉下载部分，避免再次触发网络请求）
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit=True, is_train=False, pretrained=True):
    ...
    if name in _MODELS:
        if name == "ViT-B/16":
            model_path = "/public/home/hpc202520500193/GIFT/cache/clip/ViT-B-16.pt"
        else:
