...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_attn: bool = False):
        attn_out, attn_weights = self.attention(self.ln_1(x), return_attn=return_attn)
        x = x + attn_out
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_attn: bool = False):
        ...
        for block in self.resblocks:
            x, attn_weight = block(x, return_attn=return_attn)
            if return_attn:
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_attn=False):
        if isinstance(self.visual, ModifiedResNet):
            return self.visual(image.type(self.dtype))
        else:
            return self.visual(image.type(self.dtype), return_attn=return_attn)
